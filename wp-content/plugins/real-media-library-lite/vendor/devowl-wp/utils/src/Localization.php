...
trait Localization
{
     ...
     * @return string
     */
    protected abstract function override($locale);
    /**
    ...
    {
        list($mofile, $packagePath, $packageDomain, $packageDomainFilePrefix) = $this->packageInfoParserForOverrides(self::$PACKAGE_INFO_BACKEND, $mofile, $domain);
        if ($domain === $packageDomain && !\is_readable($mofile) && \substr($mofile, 0, \strlen($packagePath)) === $packagePath) {
