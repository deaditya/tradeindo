...
class WelcomePage
{
    ...
    const PLUGIN_SLUG_FIXER = ['real-category-library-lite' => 'real-category-library', 'real-media-library-lite' => 'real-media-library', 'real-thumbnail-generator-lite' => 'real-thumbnail-generator'];
    private $initiator;
    /**
     * C'tor.
     *
    ...
    <div class="about-wrap-content">
    	...
    	<div class="feature-section has-%keyFeaturesCount-columns is-fullwidth">
    		%featuresHtml
        </div>
        ...
        <div class="feature-section has-1-columns column-links real-utils-newsletter-box">
            ...
            'href="https://devowl.io/privacy-policy/" target="_blank"'
        ) . '</span>
                    </label>
                </div>
            </p>
        ...
        $heroButton = isset($heroButton) ? \sprintf('<a href="%s" target="_blank" class="button button-primary button-hero">%s</a>', add_query_arg('feature', 'welcome-page', $heroButton[1]), $heroButton[0]) : '';
        $data = ['%slug' => $this->getInitiator()->getPluginSlug(), '%title' => $pluginData['Name'], '%version' => $pluginData['Version'], '%subtitle' => \str_replace('<cite>', \sprintf('<br /><cite>', \MatthiasWeb\RealMediaLibrary\Vendor\DevOwl\RealUtils\Core::getInstance()->getBaseAssetsUrl('devowl.io-logo.svg')), $pluginData['Description']), '%logoUrl' => $this->getInitiator()->getAssetsUrl('logo.svg'), '%supportLink' => $this->getInitiator()->getSupportLink(), '%rateLink' => $this->getInitiator()->getRateLink(), '%keyFeaturesCount' => $keyFeaturesCount, '%heroButton' => $heroButton, '%featuresHtml' => $this->transformKeyFeaturesHtml(), '%currentEmail' => esc_attr(get_userdata(get_current_user_id())->user_email), '%ourPluginsHeader' => $ourPluginsOutput['header']];
    ...
    /**
     * Get key features and transform them to key features HTML.
     */
    ...
    %s
    %s
</div>', $feature['image'], $feature['title'], $feature['description'], $availableIn, $highlightBadge);
        }
