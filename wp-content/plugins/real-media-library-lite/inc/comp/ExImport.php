...
class ExImport implements IOverrideExImport {
    ...
    public function options_register() {
        add_settings_section(
            ...
            [Options::getInstance(), 'empty_callback'],
            'media'
        );
    ...
    public function getFolders() {
        ...
        $metas = Util::getInstance()->group_by(
    ...
    public function getHierarchicalTaxos() {
        ...
        return array_unique(array_merge($taxonomy_sql, $taxos));
    }
    ...
    public function isImportTaxNoticeDismissed($set = null) {
        $transient = RML_OPT_PREFIX . '_importTaxNotice';
        $value = '1';

        if ($set !== null) {
            if ($set) {
                set_site_transient($transient, $value, 365 * constant('DAY_IN_SECONDS'));
            } else {
