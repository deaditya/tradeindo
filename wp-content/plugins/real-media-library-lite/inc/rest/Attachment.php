...
class Attachment
{
    ...
    {
        if ($this->isPro()) {
            register_rest_route(\MatthiasWeb\RealMediaLibrary\rest\Service::LEGACY_NAMESPACE, '/attachments/(?P<id>\\d+)', ['methods' => 'PUT', 'callback' => [$this, 'updateItem']]);
        }
        register_rest_route(\MatthiasWeb\RealMediaLibrary\rest\Service::LEGACY_NAMESPACE, '/attachments/(?P<id>\\d+)/shortcutInfo', ['methods' => 'GET', 'callback' => [$this, 'routeShortcutInfo']]);
        register_rest_route(\MatthiasWeb\RealMediaLibrary\rest\Service::LEGACY_NAMESPACE, '/attachments/bulk/move', ['methods' => 'PUT', 'callback' => [$this, 'routeBulkMove']]);
    }
    /**
    ...
    public function routeShortcutInfo($request)
    {
        ...
        return new \WP_REST_Response(['html' => \MatthiasWeb\RealMediaLibrary\attachment\CustomField::getInstance()->getShortcutInfoContainer($id)]);
    }
    /**
     * See API docs.
     *
    ...
    public function routeBulkMove($request)
    {
        ...
        if (\is_array($result)) {
            ...
            return new \WP_REST_Response(\MatthiasWeb\RealMediaLibrary\rest\Service::responseModify(self::MODIFIER_TYPE_BULK_MOVE, ['counts' => \MatthiasWeb\RealMediaLibrary\attachment\Structure::getInstance()->getFolderCounts()]));
        }
    }
}
