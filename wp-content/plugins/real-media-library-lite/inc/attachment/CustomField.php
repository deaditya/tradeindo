...
class CustomField {
    ...
    public function attachment_fields_to_edit($form_fields, $post) {
        ...
        $form_fields['rml_dir'] = [
            ...
            'input' => 'html',
            'html' => '<div class="rml-compat-preUploadUi">' . $selector . '</div><p class="description">' . $textToMove . '</p>' . $appendHTML
        ];
            ...
            if ($shortcut > 0) {
                ...
                if ($shortcutsCnt > 0) {
                    ...
                    foreach ($shortcuts as $value) {
                        $folderName =
                            $value['folderId'] === '-1'
                                ? wp_rml_get_object_by_id(-1)->getName(true)
                                : htmlentities($value['name']);
                        $output .= '<div>';
                        $output .=
                            $folderName .
                            ' (<a target="_blank" href="' .
                            admin_url('post.php?post=' . $value['attachment'] . '&action=edit') .
    ...
    public static function getInstance() {
        return self::$me === null ? (self::$me = new CustomField()) : self::$me;
    }
}
